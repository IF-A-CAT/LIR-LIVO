...
bool PLNet::deserialize_engine() {
  ...
  if (file0.is_open()) {
    ...
    if (engine0_ == nullptr) {
      std::cout << "Failed to deserialize engine, you may change the gpu device, please delete the old engine and regenerate the new engine." << std::endl;
      delete[] model_stream;
    }
    delete[] model_stream;  // when change the GPU type, need to delete the old engine and regenerate the new engine
    deserialize0 = true;
  }
  if (file1.is_open()) {
